...
export function EventCuisine({ cuisines, menus, getImageUrl }: EventCuisineProps) {
  ...
  return (
    <section id="cuisines" className="scroll-mt-32 pt-16 md:pt-24 w-full border-t border-gray-100">
      <SectionHeading 
        title="Global Cuisine Portfolio" 
        action={
      ...
      {menus && menus.length > 0 && (
        <div className="mt-12 md:mt-16 pt-10 border-t border-gray-100">
          <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-6">Download Banquet Menus</h4>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {menus.map((m) => (
              <a 
                ...
                target="_blank" 
                rel="noreferrer" 
                className="flex items-center gap-4 bg-gray-50/50 border border-gray-100 hover:border-[#FF9530] hover:bg-orange-50/30 p-5 rounded-[1.5rem] transition-all group shadow-[0_2px_10px_rgba(0,0,0,0.02)]"
              >
                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shrink-0 border border-gray-200 group-hover:border-[#FF9530] transition-colors">
                  <FileText className="w-4 h-4 text-gray-400 group-hover:text-[#FF9530]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-gray-900 text-[13px] leading-tight mb-1.5 truncate">{m.name}</p>
                  <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest truncate leading-none">{m.suitable_for || 'Curated Menu'}</p>
                </div>
                <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center shrink-0 group-hover:bg-[#FF9530] transition-colors">
                <Download className="w-3.5 h-3.5 text-gray-400 group-hover:text-white" />
                </div>
              </a>
            ))}
          </div>
