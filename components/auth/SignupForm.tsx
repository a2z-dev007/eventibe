...
const SuccessModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-primary-navy border border-white/10 rounded-[32px] sm:rounded-[40px] shadow-2xl max-w-md w-full p-10 relative overflow-hidden group">
        <div className="absolute -top-24 -right-24 w-48 h-48 bg-accent-orange/20 rounded-full blur-3xl group-hover:bg-accent-orange/30 transition-all duration-700" />
...
const SignupForm = () => {
  ...
  const onSubmit = useCallback(async (data: SignupFormData) => {
    if (isCaptchaEnabled && !recaptchaVerified) {
  ...
  }, [isCaptchaEnabled, recaptchaVerified, recaptchaToken, reset, resetRecaptcha])
    ...
    <div className="w-full mx-auto px-2 sm:px-4 py-4 sm:py-8">
      <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[32px] sm:rounded-[40px] p-5 sm:p-10 shadow-2xl text-white">
        <div className="text-center flex flex-col items-center mb-6 sm:mb-8">
          <div className="relative w-32 h-10 mb-3 sm:mb-4">
            <Image
              src={logoImg}
              alt="Eventibe Logo"
              fill
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8 bg-white/5 px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl border border-white/10">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 ml-4">
            <div className={`h-7 w-7 sm:h-8 sm:w-8 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm ${currentStep >= 1 ? "bg-accent-orange text-white shadow-lg shadow-orange-500/20" : "bg-white/10 text-white/30"}`}>1</div>
            <div className={`h-[1px] w-4 sm:w-6 rounded-full ${currentStep >= 2 ? "bg-accent-orange" : "bg-white/10"}`}></div>
            <div className={`h-7 w-7 sm:h-8 sm:w-8 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm ${currentStep >= 2 ? "bg-accent-orange text-white shadow-lg shadow-orange-500/20" : "bg-white/10 text-white/30"}`}>2</div>
          </div>
        </div>
          ...
          {currentStep === 1 ? (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-4 sm:space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5 sm:space-y-2 group">
                  ...
                  <div className="relative">
                    <User className="absolute left-3.5 top-3.5 w-4 h-4 sm:w-5 sm:h-5 text-white/40 group-focus-within:text-accent-orange transition-colors" />
                    <Input {...register("firstName")} placeholder="Enter First Name" className={`pl-10 sm:pl-11 h-11 sm:h-12 bg-white/10 border-white/20 text-white placeholder:text-white/30 rounded-xl focus-visible:ring-2 focus-visible:ring-accent-orange/20 focus-visible:border-accent-orange focus:border-accent-orange transition-all text-sm sm:text-base ${errors.firstName ? 'border-red-400/50' : ''}`} />
                  </div>
                ...
                <div className="space-y-1.5 sm:space-y-2 group">
                  ...
                  <div className="relative">
                    <User className="absolute left-3.5 top-3.5 w-4 h-4 sm:w-5 sm:h-5 text-white/40 group-focus-within:text-accent-orange transition-colors" />
                    <Input {...register("lastName")} placeholder="Enter Last Name" className={`pl-10 sm:pl-11 h-11 sm:h-12 bg-white/10 border-white/20 text-white placeholder:text-white/30 rounded-xl focus-visible:ring-2 focus-visible:ring-accent-orange/20 focus-visible:border-accent-orange focus:border-accent-orange transition-all text-sm sm:text-base ${errors.lastName ? 'border-red-400/50' : ''}`} />
                  </div>
              ...
              <div className="space-y-1.5 sm:space-y-2 group">
                ...
                <div className="relative">
                  <Mail className="absolute left-3.5 top-3.5 w-4 h-4 sm:w-5 sm:h-5 text-white/40 group-focus-within:text-accent-orange transition-colors" />
                  <Input {...register("email")} type="email" onBlur={handleEmailBlur} placeholder="example@email.com" className={`pl-10 sm:pl-11 h-11 sm:h-12 bg-white/10 border-white/20 text-white placeholder:text-white/30 rounded-xl focus-visible:ring-2 focus-visible:ring-accent-orange/20 focus-visible:border-accent-orange focus:border-accent-orange transition-all text-sm sm:text-base ${errors.email || emailVerification.isValid === false ? 'border-red-400/50' : ''}`} />
                  {emailVerification.isChecking && <div className="absolute right-4 top-3.5"><Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin text-accent-orange" /></div>}
                  {emailVerification.isValid === true && <div className="absolute right-4 top-3.5"><CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-green-450" /></div>}
                </div>
                {errors.email && <p className="text-red-400 text-[10px] sm:text-[11px] font-medium ml-1">{errors.email.message}</p>}
                {emailVerification.message && (
                  <p className={`text-[10px] sm:text-[11px] font-medium ml-1 flex items-center gap-1 ${emailVerification.isValid ? 'text-green-450' : 'text-red-400'}`}>
              ...
              <div className="space-y-1.5 sm:space-y-2 group">
                ...
                <div className="relative">
                  <Phone className="absolute left-3.5 top-3.5 w-4 h-4 sm:w-5 sm:h-5 text-white/40 group-focus-within:text-accent-orange transition-colors" />
                  <Input {...register("phone")} placeholder="10-15 digit mobile number" className={`pl-10 sm:pl-11 h-11 sm:h-12 bg-white/10 border-white/20 text-white placeholder:text-white/30 rounded-xl focus-visible:ring-2 focus-visible:ring-accent-orange/20 focus-visible:border-accent-orange focus:border-accent-orange transition-all text-sm sm:text-base ${errors.phone ? 'border-red-400/50' : ''}`} maxLength={15} onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9]/g, '').slice(0, 15) }} />
                </div>
                {errors.phone && <p className="text-red-400 text-[10px] sm:text-[11px] font-medium ml-1">{errors.phone.message}</p>}
              </div>

              <Button type="button" onClick={nextStep} className="w-full bg-cta-gradient h-12 sm:h-14 rounded-xl sm:rounded-2xl text-base sm:text-lg font-bold text-white shadow-lg shadow-orange-500/20 active:scale-95 transition-all flex items-center justify-center gap-2">Continue <ChevronRight className="w-5 h-5" /></Button>
            </div>
          ) : (
            <div className="animate-in fade-in slide-in-from-left-4 duration-500 space-y-4 sm:space-y-6">
              <div className="space-y-1.5 sm:space-y-2 group">
                ...
                <div className="relative">
                  <Lock className="absolute left-3.5 top-3.5 w-4 h-4 sm:w-5 sm:h-5 text-white/40 group-focus-within:text-accent-orange transition-colors" />
                  <Input {...register("password")} type={showPassword ? "text" : "password"} placeholder="Minimum 6 characters" className={`pl-10 sm:pl-11 pr-11 h-11 sm:h-12 bg-white/10 border-white/20 text-white placeholder:text-white/30 rounded-xl focus-visible:ring-2 focus-visible:ring-accent-orange/20 focus-visible:border-accent-orange focus:border-accent-orange transition-all text-sm sm:text-base ${errors.password ? 'border-red-400/50' : ''}`} />
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 flex items-center space-x-3 transition-colors hover:bg-white/10">
                <Checkbox id="terms" checked={watch("terms")} onCheckedChange={(val) => setValue("terms", val as boolean)} className="border-white/30 data-[state=checked]:bg-accent-orange data-[state=checked]:border-accent-orange" />
                <label htmlFor="terms" className="text-[10px] sm:text-xs font-semibold text-white/70 leading-relaxed cursor-pointer select-none">I agree to the <Link href={LINKS.TERMS_CONDITIONS} target="_blank" className="text-accent-orange hover:text-accent-orange/80">Terms</Link> & <Link href={LINKS.PRIVACY_POLICY} target="_blank" className="text-accent-orange hover:text-accent-orange/80">Privacy Policy</Link></label>
              </div>
              {errors.terms && <p className="text-red-400 text-[10px] sm:text-[11px] font-bold ml-1">{errors.terms.message}</p>}

              {isCaptchaEnabled && (
                <div className="flex justify-center my-4 sm:my-6">
                  <RecaptchaV2 containerId="recaptcha-signup" />
                </div>
              )}
              ...
              <div className="flex gap-3 sm:gap-4">
                ...
                <Button type="submit" disabled={isLoading || (isCaptchaEnabled && !recaptchaVerified)} className="flex-[2] bg-cta-gradient h-12 sm:h-14 rounded-xl sm:rounded-2xl text-base sm:text-lg font-bold text-white shadow-lg shadow-orange-500/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">{isLoading ? <><Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> Signing up...</> : "Create Account"}</Button>
              </div>
            </div>
          )}
        </form>
        ...
        <div className="mt-6 sm:mt-8 text-center bg-white/5 py-4 sm:py-5 rounded-2xl border border-white/10">
          <p className="text-white/60 text-[11px] sm:text-sm font-medium px-2">
            Already have an account? <Link href="/login" className="text-accent-orange font-extrabold hover:underline inline-block sm:inline whitespace-nowrap hover:text-accent-orange/80">Sign in here</Link>
          </p>
        </div>
      </div>
