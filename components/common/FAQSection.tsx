...
const FAQSection = ({
    ...
    // Fetch FAQs from API
    const { data: faqResponse, isLoading, error } = useQuery({
        queryKey: ['faqs', listingId, cityId, stateId, countryId],
        queryFn: () => getFAQs({
            listing: listingId,
            city: cityId,
            state: stateId,
        ...
        enabled: !!(listingId || cityId || stateId || countryId),
    })

    const faqs = faqResponse?.data?.records || []
    ...
    return (
        <section className="py-16 bg-white rounded-2xl border border-gray-200">
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="space-y-4  overflow-y-auto px-4" style={{ maxHeight: '30vh' }}>
                    {faqs.map((faq: any, index: number) => (
                        <div key={faq.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden transition-all duration-300">
                            ...
                            <div
                                className={`grid gradient-orange-bg text-white transition-all duration-300 ease-in-out ${openItem === index
                                        ? 'grid-rows-[1fr]'
                                        : 'grid-rows-[0fr]'
                                    }`}
                            >
