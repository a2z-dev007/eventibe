...
export default async function CityWeddingVenuesPage({ params }: { params: Promise<{ city: string }> }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      <CommonHero
        badgeText={`Dream Wedding in ${cityData.name}`}
        badgeIcon="heart"
        titleMain="Wedding Venues in"
        titleHighlight={cityData.name}
        subtitle={`Browse our curated selection of premium wedding spaces in ${cityData.name}. Perfect for your dream wedding.`}
      ...
      <div className="bg-slate-50 py-20 border-t border-gray-100">
        <div className="container mx-auto px-4 md:px-6">
          <ListingGrid>
            {venues.map((venue) => (
              <VenueCard key={venue.id} venue={venue} accent="rose" />
            ))}
            {venues.length === 0 && (
