...
const filePath = path.join('d:', 'ClientProject', 'eventibe', 'app', 'privacy-policy', 'page.tsx');
...
const sections = [
  ...
  { id: "share", title: "Sharing Information", icon: <Lock size={24} /> },
  { id: "retention", title: "Data Retention", icon: <Clock size={24} /> },
  { id: "security", title: "Data Security", icon: <ShieldCheck size={24} /> },
...
`;
    content = content.replace('export default function PrivacyPolicyPage() {', sectionsCode + '\nexport default function PrivacyPolicyPage() {');
}
...
if (!content.includes('const [activeSection')) {
    content = content.replace('export default function PrivacyPolicyPage() {', 'export default function PrivacyPolicyPage() {\n' + hookCode);
}

// 4. Hero section center
const heroOriginal = \`        <div className="container mx-auto px-4 md:px-6 relative z-10 text-center md:text-left">
...
const heroNew = \`        <div className="container mx-auto px-4 md:px-6 relative z-10 text-white flex flex-col items-center text-center">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
...
content = content.replace(heroOriginal, heroNew);

// 5. Sidebar and structure
const sideMatch = content.match(/<aside className="hidden lg:block lg:col-span-4 sticky top-24 self-start space-y-4">.*?<\\/aside>/s);
if (sideMatch) {
    const asideNew = \`            <style
    ...
    content = content.replace(sideMatch[0], asideNew);
}
...
// 6. Remove headings numbers
const headingsRegex = /(\\d+\\.\\s+)(ABOUT US|INFORMATION WE COLLECT|HOW WE USE INFORMATION|SHARING INFORMATION|DATA RETENTION|DATA SECURITY|THIRD-PARTY LINKS|CHILDREN’S PRIVACY|YOUR RIGHTS|INTERNATIONAL TRANSFER|BUSINESS TRANSFERS|CHANGES TO THIS POLICY|CONTACT INFORMATION|CONSENT|THIRD-PARTY CONTENT & LINKS)/g;
content = content.replace(headingsRegex, '$2');
content = content.replace(/>\\d+\\.\\d+\\s+([A-Z])/g, '>$1'); // e.g. "2.1 Information" -> "Information"
content = content.replace(/>\\d+\\.\\s+([A-Z])/g, '>$1'); // handle other cases just in case "1. About"
